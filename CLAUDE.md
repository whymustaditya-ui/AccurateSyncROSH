...
| `Rekap.gs` | 🏢 REKAP TAGIHAN CORPORATE (**seksi di bawah tabel `🧾 Tagihan Non-Sales`, master-only**, BUILT 2026-09-13): untuk customer yang bayar per rekap tagihan + No. Surat Jalan (`CONFIG.REKAP_CUSTOMERS`: grup Sinar Rasa ×3, The Akara, Legenda Kuliner, LBFoods, BWC Kuningan City; match *contains* setelah `_rekapNorm` buang PT/CV/tanda baca). Menarik **SEMUA faktur belum lunas** customer itu tanpa batas umur (termasuk yang sudah di Ade, status `Lewat N hari (di Ade)`), blok per customer + SUBTOTAL + TOTAL CORPORATE; kolom `No. Invoice · Tgl Terbit · No. Surat Jalan · Jatuh Tempo · Hari Lewat JT · Nilai Faktur · Sudah Bayar · Outstanding · Status · 📄 Invoice · Sales / Sumber · No. Telp · Loyalitas (4bln)` (13 kolom; 11-13 menumpang kolom spacer + side panel, aman karena seksi jauh di bawah baris 25). **Surat jalan hanya ada di `sales-invoice/detail.do`** (list.do tidak punya) → `attachSuratJalan` tarik per faktur, dibatasi `REKAP_SJ_MAX` 60 / `REKAP_SJ_BUDGET_MS` 60s per run, cache tersembunyi `_SjCache` (SJ statis per faktur, satu kali tarik; kosong = belum tertarik, `(tanpa SJ)` = faktur memang tanpa SJ). **CONFIRMED diag 2026-09-13: SJ = `detailItem[].deliveryOrder.number`** (header `deliveryOrder` cuma boolean, diabaikan; `_sjClean` membuang token "false" dari cache lama). Ganti mapping → menu **Diag surat jalan fields** lalu **Rebuild cache Surat Jalan**. Dibangun di `fullSync` sesudah `buildInvoiceLain` (fail-soft), ditulis oleh `writeInvoiceLainTab(list, rekap, today)` dalam `try` sendiri; range Hari Lewat JT seksi ikut rules warna tab. Customer tanpa faktur terbuka tetap tampil (baris catatan) supaya salah nama ketahuan. Scope `sales_invoice_view` (sudah ada) |
