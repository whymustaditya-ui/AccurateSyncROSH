...
- **Sales KPI:** THP = base 3.5jt + tunjangan(score×3.5jt, cap 106%) + komisi(1.25% on basis >100jt). Weights: omzet .45 / cashflow .25 / diskon .20 / NOO .10.
  - **Basis komisi = kas PRE-HANDOVER saja sejak Agustus 2026** (`CONFIG.SALES_COMMISSION_PREHANDOVER_FROM: '2026-08'`). Kas yang cair setelah `handoverDate` (H+15) ditagih Ade dan sudah dibayar komisi 1,5–3,5% ke dia; dulu kas yang sama juga menambah komisi Deden → satu penagihan dibayar dua kali. **Omzet (bobot .45) TETAP seluruh kas** — Deden yang menciptakan penjualannya; yang berubah cuma basis komisi. Floor **tetap 100jt** (keputusan Bro 2026-08-02) → konsekuensi diterima sadar: komisi jadi jarang cair (kas pre-handover Juli Rp98,3jt, di bawah floor → komisi Rp0). Opsi floor 75jt (netral ke gaji) ditolak. **Tanggal efektif WAJIB ada**: `restampPreviousMonth` menghitung ulang bulan lalu selama 7 hari pertama bulan baru, tanpa guard ini Juli ikut kena dan komisi Rp294.872 yang sudah jadi hak Deden hilang surut. Kosongkan konstantanya untuk kembali ke aturan lama. Rasional lengkap + angka: `ROSH Finance/2026-08-02_MEMO_Komisi-Sales-Kas-Post-Handover.html`. Ukur ulang tiap kuartal: menu **Simulasi komisi pre-handover** (`diagKomisiPreHandover`, read-only).
